...
__all__ = ['MobileNetV2', 'mobilenetv2_050', 'mobilenetv2_075', 'mobilenetv2_100']
...
class MobileNetV2(nn.Module):
    ...
    def _forward_impl(self, x,is_feat=False):
        ...
        # 分类
        x = nn.functional.adaptive_avg_pool2d(feat, 1).reshape(feat.shape[0], -1)
        x = self.classifier(x)

        # 根据 is_feat 参数决定是否返回特征
        if is_feat:
            return feat, x
        return x
...
def mobilenetv2_075(**kwargs):
    return mobilenet_v2(width_mult=0.75, **kwargs)
