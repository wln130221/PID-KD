...
class VGG(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        # 使用Canny边缘提取
        x_canny = self.canny_edge_detection(x)

        # 继续特征提取
        x = F.relu(self.block0(x_canny))
        f0 = x
        x = self.pool0(x)
        x = self.block1(x)
        f1_pre = x
        ...
        if is_feat:
            ...
            return x  # 返回边缘特征图x_canny

    def canny_edge_detection(self, x):
        # 将张量转换为numpy数组以使用OpenCV
        x_np = x.detach().cpu().numpy()  # 假设输入是[batch_size, channels, height, width]
        batch_size, channels, height, width = x_np.shape

        edges = np.zeros_like(x_np)
        for i in range(batch_size):
            for j in range(channels):
                # 将通道转换为灰度图像
                gray_image = cv2.cvtColor(x_np[i].transpose(1, 2, 0), cv2.COLOR_RGB2GRAY)
                if gray_image.dtype != np.uint8:
                    # Scale to 0-255 if the image is not in uint8 format
                    gray_image = (gray_image * 255).astype(np.uint8)
                # 应用Canny边缘检测
                edges[i, j] = cv2.Canny(gray_image, 100, 200)

        # 转换回张量
        return torch.from_numpy(edges).to(x.device)
